...
static const char *driverName = "drvNSLS2I_EM";

// Global variable containing pointer to your driver object
class drvNSLS2I_EM *pdrvNSLS2I_EM;
...
*
*********************************************/
asynStatus drvNSLS2I_EM::readMeter(int *adcbuf)
{
...
{
    while(1) { /* Do forever */
        if (pdrvNSLS2I_EM->isAcquiring()) pdrvNSLS2I_EM->callbackFunc();
...
*
********************************************/
asynStatus drvNSLS2I_EM::pl_open(int *fd) {
...
{
    pdrvNSLS2I_EM->callbackFunc();
}
#endif


// The constructor for your driver
drvNSLS2I_EM::drvNSLS2I_EM(const char *portName, int ringBufferSize) : drvQuadEM(portName, ringBufferSize)
{
    int i;
    float fsd;
...
// Set the global pointer
    ...
    //const char *functionName = "drvNSLS2I_EM";
    ...
    for (i=0;i<QE_MAX_INPUTS;i++){
        scaleFactor_[i][0] = 0.1/fsd;
        scaleFactor_[i][1] = 1.0/fsd;
        scaleFactor_[i][2] = 10.0/fsd;
        scaleFactor_[i][3] = 100.0/fsd;
        scaleFactor_[i][4] = 1000.0/fsd;
	scaleFactor_[i][5] = 10000.0/fsd;
    }
//    printf("scaleFactors: %g  %g  %g  %g\n", scaleFactor_[0][0], scaleFactor_[0][1], scaleFactor_[0][2],scaleFactor_[0][3] );
    ...
    getFirmwareVersion();
    setIntegerParam(P_Model, QE_ModelNSLS2I_EM);
    callParamCallbacks();
}

drvNSLS2I_EM::~drvNSLS2I_EM()
{
...
asynStatus drvNSLS2I_EM::writeInt32(asynUser *pasynUser, epicsInt32 value)
{
...
asynStatus drvNSLS2I_EM::getBounds(asynUser *pasynUser, epicsInt32 *low, epicsInt32 *high)
{
    int function = pasynUser->reason;
...
void drvNSLS2I_EM::callbackFunc()
{
...
asynStatus drvNSLS2I_EM::setAcquire(epicsInt32 value)
{
...
asynStatus drvNSLS2I_EM::setAcquireParams()
{
    int numAverage;
    int valuesPerRead;
    double sampleTime;
...
asynStatus drvNSLS2I_EM::setAveragingTime(epicsFloat64 value)
{
...
/** Sets the values per read.
  ...
  */
asynStatus drvNSLS2I_EM::setValuesPerRead(epicsInt32 value) 
{
    return setAcquireParams();
}

asynStatus drvNSLS2I_EM::initDAC()
{
...
asynStatus drvNSLS2I_EM::setDAC(int channel, int value)
{
    int dacVolts;
...
asynStatus drvNSLS2I_EM::setBiasVoltage(epicsFloat64 value)
{
...
asynStatus drvNSLS2I_EM::setRange(int channel, epicsInt32 value)
{
...
int i;
 ...
 switch(channel){
   case 0:
    switch(value){
       case 0:
         fpgabase_[GAINREG] = (fpgabase_[GAINREG]&0xffffff00)|0x00000001;
        break;
       case 1:
         fpgabase_[GAINREG] = (fpgabase_[GAINREG]&0xffffff00)|0x00000002;
         break;
       case 2:
         fpgabase_[GAINREG] = (fpgabase_[GAINREG]&0xffffff00)|0x00000004;
         break;
       case 3:
         fpgabase_[GAINREG] = (fpgabase_[GAINREG]&0xffffff00)|0x00000008;
         break;
       case 4:
         fpgabase_[GAINREG] = (fpgabase_[GAINREG]&0xffffff00)|0x00000010;
         break;
	 }
    break;
  case 1:
    switch(value){
       case 0:
         fpgabase_[GAINREG] = (fpgabase_[GAINREG]&0xffff00ff)|0x00000100;
        break;
       case 1:
         fpgabase_[GAINREG] = (fpgabase_[GAINREG]&0xffff00ff)|0x00000200;
         break;
       case 2:
         fpgabase_[GAINREG] = (fpgabase_[GAINREG]&0xffff00ff)|0x00000400;
         break;
       case 3:
         fpgabase_[GAINREG] = (fpgabase_[GAINREG]&0xffff00ff)|0x00000800;
         break;
       case 4:
         fpgabase_[GAINREG] = (fpgabase_[GAINREG]&0xffff00ff)|0x00001000;
         break;
	 }
    break;
  case 2:	 
    switch(value){
       case 0:
         fpgabase_[GAINREG] = (fpgabase_[GAINREG]&0xff00ffff)|0x000010000;
        break;
       case 1:
         fpgabase_[GAINREG] = (fpgabase_[GAINREG]&0xff00ffff)|0x000020000;
         break;
       case 2:
         fpgabase_[GAINREG] = (fpgabase_[GAINREG]&0xff00ffff)|0x000040000;
         break;
       case 3:
         fpgabase_[GAINREG] = (fpgabase_[GAINREG]&0xff00ffff)|0x000080000;
         break;
       case 4:
         fpgabase_[GAINREG] = (fpgabase_[GAINREG]&0xff00ffff)|0x000100000;
         break;
	 }
    break;
  case 3:
    switch(value){
       case 0:
         fpgabase_[GAINREG] = (fpgabase_[GAINREG]&0x00ffffff)|0x001000000;
        break;
       case 1:
         fpgabase_[GAINREG] = (fpgabase_[GAINREG]&0x00ffffff)|0x002000000;
         break;
       case 2:
         fpgabase_[GAINREG] = (fpgabase_[GAINREG]&0x00ffffff)|0x004000000;
         break;
       case 3:
         fpgabase_[GAINREG] = (fpgabase_[GAINREG]&0x00ffffff)|0x008000000;
         break;
       case 4:
         fpgabase_[GAINREG] = (fpgabase_[GAINREG]&0x00ffffff)|0x010000000;
         break;
	 }
    break;
   }
...
asynStatus drvNSLS2I_EM::readStatus()
{
    return asynSuccess;
}

asynStatus drvNSLS2I_EM::getFirmwareVersion()
{
 int fver, i;
 char tmpstr[256];
     for(i=0;i<256;i++){
...
asynStatus drvNSLS2I_EM::reset()
{
    return asynSuccess;
}

void drvNSLS2I_EM::report(FILE *fp, int details)
{
...
/** EPICS iocsh callable function to call constructor for the drvNSLS_EM class.
  ...
  */
int drvNSLS2I_EMConfigure(const char *portName, int ringBufferSize)
{
    new drvNSLS2I_EM(portName, ringBufferSize);
    return(asynSuccess);
}
...
static const iocshFuncDef initFuncDef = {"drvNSLS2I_EMConfigure",3,initArgs};
static void initCallFunc(const iocshArgBuf *args)
{
    drvNSLS2I_EMConfigure(args[0].sval, args[1].ival);
}

void drvNSLS2I_EMRegister(void)
{
    iocshRegister(&initFuncDef,initCallFunc);
}

epicsExportRegistrar(drvNSLS2I_EMRegister);
